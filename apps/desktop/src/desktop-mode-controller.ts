...
export interface DesktopModeControllerOptions {
   ...
   * can supply is missing, such as no managed Harness version being installed.
   * The controller renders the setup phase instead of creating the surface and
   * retries once the prerequisite appears; the shell owns the localized copy.
...
export interface DesktopModeController {
  /** Create Harness and the initially selected Chat surface once. */
  start(): Promise<void>
  /** Persist a selection, creating its surface when absent. */
  select(mode: DesktopMode): Promise<void>
  /** Recreate a surface marked failed, or start one withheld in the setup phase. */
  retry(mode: DesktopMode): Promise<void>
  /** Dispose one surface and create it again, stopping the process it owns. */
  restart(mode: DesktopMode): Promise<void>
  /** Retain and apply content bounds to every live surface. */
  resize(bounds: DesktopContentBounds): void
  /** Reload a live Chat renderer without changing its session. */
  reloadChat(): void
  /** Dispose Chat, clear its browsing data, and recreate it when selected. */
  clearChatData(): Promise<void>
  /** Replace the HTTPS URL awaiting explicit browser confirmation. */
  offerExternalUrl(url: string): void
  /** Open the current offered URL and clear it only if no newer offer arrived. */
  openPendingExternal(): Promise<void>
  /** Mark one mode failed and dispose only that mode's surface. */
  fail(mode: DesktopMode, error: Error): Promise<void>
  /** Return a detached copy of the current selection and phases. */
  snapshot(): DesktopModeSnapshot
  /** Reject new transitions and join both surface disposers. */
...
export function createDesktopModeController(options: DesktopModeControllerOptions): DesktopModeController {
  ...
  function hasShutdownStarted(): boolean {
    return shuttingDown
  }
  ...
  async function createSurface(mode: DesktopMode, replaceLoading = false): Promise<void> {
    ...
    if (mode === 'harness' && options.harnessSetupRequired?.() === true) {
      generations[mode] += 1
      setStatus(mode, { phase: 'setup' })
      return
    }
    ...
    if (hasShutdownStarted() || generations[mode] !== generation) {
  ...
  return {
    ...
    async retry(mode: DesktopMode): Promise<void> {
      return enqueue(async () => {
        const phase = statuses[mode].phase
        if (shuttingDown || (phase !== 'failed' && phase !== 'setup')) return
        await createSurface(mode)
      })
    },

    async restart(mode: DesktopMode): Promise<void> {
      return enqueue(async () => {
        if (shuttingDown) return
        generations[mode] += 1
        await disposeSurface(mode)
        statuses[mode] = { phase: 'idle' }
        await createSurface(mode)
      })
    },
