...
import type { HostReadiness, HostSupervisor } from './host-supervisor.ts'
...
export interface HarnessSurfaceOptions {
  ...
  /**
   * Attach the per-launch Host credential to this surface's origin, at the
   * Electron request layer.
   * @param origin - the loopback origin the Host reported at readiness.
   * @returns a disposer detaching the credential again.
   */
  readonly attachApiAuth?: (origin: string) => () => void
}
...
export async function createHarnessSurface(options: HarnessSurfaceOptions): Promise<DesktopThemedSurface> {
  ...
  try {
    readiness = await options.host.start()
  } catch (error) {
  ...
  // Attach before any renderer request can reach the Host, and detach on every
  // path that stops this surface — a credential left injected after shutdown
  // would travel to whatever next binds that loopback port. A Host that mints
  // its own launch token authenticates the view through the readiness URL
  // instead, so the bearer stays unattached and never rides along to it.
  const detachApiAuth = readiness.token === undefined ? options.attachApiAuth?.(origin) : undefined
  const releaseApiAuth = (): void => {
    if (detachApiAuth === undefined) return
    try {
      detachApiAuth()
    } catch (error: unknown) {
      console.error('desktop Harness auth detach failed:', error)
    }
  }
  ...
  // `set` rather than `append`: the Host rejects a handoff URL carrying its
  // token twice, and the first load is what exchanges it for the cookie.
