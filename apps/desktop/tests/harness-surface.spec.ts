...
function fakeHost(readiness: HostReadiness = { origin: 'http://127.0.0.1:4173' }) {
  let unexpectedExit: ((detail: { code: number | null; signal: NodeJS.Signals | null }) => void) | undefined
  const host = {
    start: vi.fn(() => Promise.resolve(readiness)),
...
describe('Harness surface', () => {
  ...
  it('carries a Host launch token into the first load and leaves the bearer unattached', async () => {
    const { host } = fakeHost({ origin: 'http://127.0.0.1:4173', token: 'Qw3rtY-7_z9' })
    const { view, contents } = fakeView()
    const attachApiAuth = vi.fn(() => vi.fn())
    ...
    const loaded = new URL(String(contents.loadURL.mock.calls[0]?.[0]))
    expect(loaded.origin).toBe('http://127.0.0.1:4173')
    expect(loaded.searchParams.getAll('token')).toEqual(['Qw3rtY-7_z9'])
    expect(loaded.searchParams.get('dsh-desktop-embedded')).toBe('1')
    expect(attachApiAuth).not.toHaveBeenCalled()

    await surface.dispose()
  })

  it('attaches the spawn-environment bearer for a Host that mints no token', async () => {
    const { host } = fakeHost()
    const { view } = fakeView()
    const detach = vi.fn()
    const attachApiAuth = vi.fn(() => detach)
    ...
    expect(attachApiAuth).toHaveBeenCalledWith('http://127.0.0.1:4173')
    await surface.dispose()
