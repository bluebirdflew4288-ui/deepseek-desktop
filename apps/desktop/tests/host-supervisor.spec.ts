...
describe('desktop Host readiness', () => {
  ...
  it('surfaces the launch token a Harness mints for its browser handoff', () => {
    const parser = createReadinessParser()

    expect(parser.push('dsh web: http://127.0.0.1:4173/?token=Qw3rtY-7_z9\n'))
      .toEqual({ origin: 'http://127.0.0.1:4173', token: 'Qw3rtY-7_z9' })
    expect(parser.finalize()).toEqual({ origin: 'http://127.0.0.1:4173', token: 'Qw3rtY-7_z9' })
  })

  it.each([
    'dsh web: http://127.0.0.1:4173/?token=Qw3rtY-7_z9&other=1',
    'dsh web: http://127.0.0.1:4173/?state=Qw3rtY',
    'dsh web: http://127.0.0.1:4173/?token=',
    'dsh web: http://127.0.0.1:4173/?token=not%20base64url',
  ])('rejects a readiness line carrying an unusable query: %s', (line) => {
    const parser = createReadinessParser()
  ...
  it('rejects a second readiness line minting a different token for one origin', () => {
    const parser = createReadinessParser()

    expect(parser.push('dsh web: http://127.0.0.1:4173/?token=Qw3rtY\n'))
      .toEqual({ origin: 'http://127.0.0.1:4173', token: 'Qw3rtY' })
    expect(() => parser.push('dsh web: http://127.0.0.1:4173/?token=z9Ytrw\n'))
      .toThrow(/conflicting readiness URLs/iu)
  })
})
...
describe('desktop Host process', () => {
  it('opts the packaged Electron executable into its Node runtime', async () => {
    ...
    spawnDshWeb({
      ...
      cliEntry: '/Applications/DeepSeek Desktop.app/Contents/Resources/host/node_modules/@deepseek-ai/dsh/lib/bin.js',
      cwd: '/Users/tester',
      env: { DSH_DESKTOP: '1' },
  ...
  it('declines the browser handoff only for a Host that registers the option', async () => {
    const spawned = {
      stdout: { on: vi.fn(), off: vi.fn() },
      stderr: { on: vi.fn(), off: vi.fn() },
      on: vi.fn(),
    ...
    const managed = {
      nodeExecutable: '/Applications/DeepSeek Desktop.app/Contents/MacOS/DeepSeek Desktop',
      cliEntry: '/managed/versions/0.1.5-rc.1/node_modules/@deepseek-ai/dsh/lib/bin.js',
      cwd: '/Users/tester',
      env: {},
      electronRunAsNode: true,
    }
    vi.mocked(spawn).mockClear()
    spawnDshWeb({ ...managed, suppressBrowserHandoff: true })
    expect(vi.mocked(spawn).mock.calls[0]?.[1]).toContain('--no-open')

    vi.mocked(spawn).mockClear()
    spawnDshWeb(managed)
    expect(vi.mocked(spawn).mock.calls[0]?.[1]).not.toContain('--no-open')
  })
})
