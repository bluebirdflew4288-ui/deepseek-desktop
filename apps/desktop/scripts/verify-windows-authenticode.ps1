...
try {
  if (-not (Test-Path -LiteralPath $ZipPath -PathType Leaf)) {
    throw 'Signed release ZIP is missing.'
  }
  ...
  if (-not (Test-Path -LiteralPath $zipExecutable -PathType Leaf)) {
    throw 'Signed application executable is missing from the release ZIP.'
  }
  ...
  foreach ($file in $Path) {
    if (-not (Test-Path -LiteralPath $file -PathType Leaf)) {
      throw "Signed release executable is missing: $(Split-Path -Leaf $file)"
    }
    ...
    $results += [ordered]@{
      ...
      trustedChain = ($signature.Status.ToString() -ceq 'Valid' -and $signtoolSucceeded)
