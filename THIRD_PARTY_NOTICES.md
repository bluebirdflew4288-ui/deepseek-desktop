...
The inherited Harness dependency policy accepts the official `@anthropic-ai/claude-agent-sdk` package and the official Claude Code CLI/platform payloads that each version declares through `optionalDependencies`. That repository policy is not a license grant from Anthropic or a statement of endorsement. These payloads retain their own terms; version, declared-license, and payload-set changes require dependency, compatibility, terms, and notices review. They are not part of the sealed Desktop app’s fixed resources; separately installed Harness dependencies remain governed by their own terms.
...
| --- | --- | --- | --- | --- |
| [`npm`](https://github.com/npm/cli) | 11.12.1 | Artistic-2.0 | `sha512-zcoUuF1kezGSAo0CqtvoLXX3mkRqzuqYdL6Y5tdo8g69NVV3CkjQ6ZBhBgB4d7vGkPcV6TcvLi3GRKPDFX+xTA==` | executed by the packaged desktop to install and update the official `@deepseek-ai/dsh` Harness runtime |
