  ...
  switch (selected) {
    ...
    case 'check-all':
      return [
        ...
        // Advisory only: `pnpm deploy --legacy` re-resolves the packaged Host
        // closure, so this reports the real drift instead of a clean baseline.
        // Promote to a blocking gate and add it to the CI artifact aggregates
        // once the closure resolves from the lockfile.
        pnpmScript('runtime-lock', 'verify-runtime-lock', { label: 'runtime lock', allowFailure: true }),
