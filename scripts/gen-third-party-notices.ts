...
/** Pin owning the npm CLI the packaged desktop ships and executes at runtime. */
const SHIPPED_NPM_PIN = 'apps/desktop/npm-runtime.json'

/** One tool the packaged desktop ships inside its own resources. */
export interface ShippedToolRuntime {
...
/**
 * Read the pinned npm runtime the desktop packages.
 *
 * This tool is fetched by `apps/desktop/scripts/stage-npm.ts` and executed by the
 * shipped application, so it is a distributed runtime rather than a workspace
 * dependency, and no manifest in the workspace names it.
 * @returns the shipped tool's disclosed identity.
 * @throws when the pin is absent or incomplete, because omitting a shipped
 * non-permissive tool from the notices is not a recoverable silence.
 */
export function shippedNpmRuntime(): ShippedToolRuntime {
  const pin = JSON.parse(readFileSync(resolve(root, SHIPPED_NPM_PIN), 'utf8')) as Record<string, unknown>
  for (const field of ['name', 'version', 'license', 'repository', 'integrity'] as const) {
    if (typeof pin[field] !== 'string' || pin[field] === '') {
      throw new Error(`gen-third-party-notices: ${SHIPPED_NPM_PIN} has no ${field}.`)
    }
  }
  return {
    name: pin.name as string,
    version: pin.version as string,
    license: pin.license as string,
    repo: pin.repository as string,
    integrity: pin.integrity as string,
    role: 'executed by the packaged desktop to install and update the official `@deepseek-ai/dsh` Harness runtime',
  }
}
...
The inherited Harness dependency policy accepts the official \`${CLAUDE_AGENT_SDK_PACKAGE}\` package and the official Claude Code CLI/platform payloads that each version declares through \`optionalDependencies\`. That repository policy is not a license grant from Anthropic or a statement of endorsement. These payloads retain their own terms; version, declared-license, and payload-set changes require dependency, compatibility, terms, and notices review. They are not part of the sealed Desktop app’s fixed resources; separately installed Harness dependencies remain governed by their own terms.
...
export function render(): string {
  ...
  const claudeDistribution = runtimeDeps.some(
    ...
    : undefined
  const shippedNpm = shippedNpmRuntime()
...
This community DeepSeek Desktop derivative retains the upstream [MIT](LICENSE) license for MIT-covered code. Its Desktop foundation is [zcx960/deepseek-desktop](https://github.com/zcx960/deepseek-desktop) at revision \`1e189113f7edce2da71e96c3378cbc454bc32214\`. See [ACKNOWLEDGEMENTS.md](ACKNOWLEDGEMENTS.md) for source attribution and component-specific license locations. It depends on the third-party software listed below. Each project remains under its own license; nothing in this file changes those terms.
...
## Embedded source derivative

\`apps/desktop/resources/deepseek-memory/\` is a modified, Memory-only derivative of [DeepSeek++](https://github.com/zhu1090093659/deepseek-pp) 1.14.0 at revision \`0a02c72b135bf2936e11aa78fd6136931ed65908\`. It is distributed under Apache-2.0; the packaged directory includes the upstream [\`LICENSE\`](apps/desktop/resources/deepseek-memory/LICENSE) and a [\`NOTICE.md\`](apps/desktop/resources/deepseek-memory/NOTICE.md) describing the retained concepts and modifications.
...
## Shipped inside the packaged desktop application

Electron and its Node.js/Chromium components retain their upstream licenses and notices. The Electron distribution provides \`LICENSE\` and \`LICENSES.chromium.html\`. These files are absent from the sealed local macOS app, so binary redistribution requires a separate packaging correction to preserve them. This source publication does not distribute that app. Electron is listed among development dependencies because its package provides build tooling, but the Electron runtime is also shipped in the Desktop app.

The desktop application ships one pinned tool in its own resources and executes it at runtime, so it is distributed with the product rather than only used to build it. No workspace manifest names it. [\`apps/desktop/npm-runtime.json\`](apps/desktop/npm-runtime.json) pins the exact version and its sha512 integrity; [\`apps/desktop/scripts/stage-npm.ts\`](apps/desktop/scripts/stage-npm.ts) fetches that version from the official npm registry and verifies the downloaded bytes against both the pin and the metadata the registry publishes for it; [\`apps/desktop/scripts/verify-packaged-runtime.ts\`](apps/desktop/scripts/verify-packaged-runtime.ts) rejects a package whose shipped copy is not that version. Its license text ships beside it at \`Contents/Resources/npm/LICENSE\`, and explicitly states that npm’s bundled dependencies retain their respective copyright notices and license terms. Their own license files remain in the unmodified npm tree.

| Package | Version | License | Integrity | Role |
| --- | --- | --- | --- | --- |
| [\`${shippedNpm.name}\`](${shippedNpm.repo}) | ${shippedNpm.version} | ${shippedNpm.license} | \`${shippedNpm.integrity}\` | ${shippedNpm.role} |
