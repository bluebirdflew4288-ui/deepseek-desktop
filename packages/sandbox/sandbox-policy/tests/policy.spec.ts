...
describe('SandboxPolicyService', () => {
  ...
  it('resolves an agentless call to read-only instead of a write grant on the fallback root', async () => {
    const ctx = await mounted({ mode: 'workspace-write', workspaceRoot: '/fallback' })
    // A deployment default cannot lend its mode to a call that named no
    // workspace: the fallback root is still reported, but nothing is writable
    // under it, so a Host whose cwd is the user's home gains no home-wide grant.
    expect(ctx.sandboxPolicy.resolve()).toEqual({
      mode: 'read-only',
      workspaceRoot: resolve('/fallback'),
    })
    expect(writableRoots(ctx.sandboxPolicy.resolve())).toEqual([])
  })
  ...
  it('resolves each session mode and cwd together without changing the fallback', async () => {
    ...
    // The fallback root itself is unchanged; what changed is that no session
    // cwd means no write grant on it.
    expect(ctx.sandboxPolicy.resolve()).toEqual({
      mode: 'read-only',
      workspaceRoot: resolve('/fallback'),
    })
  ...
  it('reports the configured root when a session has no cwd, without making it writable', async () => {
    const ctx = await mounted({ mode: 'workspace-write', workspaceRoot: '/fallback' })
    const resolved = ctx.sandboxPolicy.resolve({ session: session('sess-no-cwd') })
    expect(resolved.workspaceRoot).toBe(resolve('/fallback'))
    expect(resolved.mode).toBe('read-only')
    expect(writableRoots(resolved)).toEqual([])
  })

  it('keeps a chosen session cwd writable under the deployment default', async () => {
    const ctx = await mounted({ mode: 'workspace-write', workspaceRoot: '/fallback' })
    const resolved = ctx.sandboxPolicy.resolve({ session: session('sess-cwd', '/projects/chosen') })
    expect(resolved).toEqual({
      mode: 'workspace-write',
      workspaceRoot: resolve('/projects/chosen'),
      sessionId: 'sess-cwd',
    })
    expect(writableRoots(resolved)).toEqual(expect.arrayContaining([resolve('/projects/chosen')]))
  })

  it('does not let an approved mode manufacture a writable root nobody chose', async () => {
    const ctx = await mounted({ mode: 'workspace-write', workspaceRoot: '/fallback' })
    const approved = ctx.sandboxPolicy.resolve({ session: session('sess-no-cwd-approved'), mode: 'workspace-write' })
    expect(approved.mode).toBe('read-only')
    expect(writableRoots(approved)).toEqual([])
  })
