...
export interface Config {
  ...
  mode?: SandboxMode
  /**
   * Fallback root reported for agentless calls and sessions without a cwd
   * (default: `process.cwd()`). It is never a write grant: with no session cwd
   * a `workspace-write` policy degrades to `read-only`, which admits no
   * writable root at all. Normal agent calls use their session cwd instead.
   */
...
export class SandboxPolicyService extends Service {
   ...
   * deployment default — but only a session cwd names a writable workspace.
   * Without one (an agentless call, or a session that never chose a directory)
   * a `workspace-write` grant would fall back to the configured root, a
   * directory nobody selected: the packaged desktop Host runs with its cwd at
   * the user's home, so that grant would be home-wide. It therefore degrades to
   * `read-only`, which admits no writable root, and the fallback root survives
   * only as the reported boundary.
   *
   * `danger-full-access` is deliberately left alone. It grants no root-scoped
   * write at all — the sandbox simply does not fence — so there is no fallback
   * root to protect, and clamping it would silently reverse an explicit opt-out
   * of the file sandbox without narrowing anything.
  ...
  resolve(request: SandboxPolicyRequest = {}): SandboxExecutionPolicy {
    ...
    const approved = request.mode ?? (session === undefined ? undefined : this.overrideOf(session)) ?? this.defaultMode
    return {
      mode: cwd === undefined && approved === 'workspace-write' ? 'read-only' : approved,
      workspaceRoot: resolveWorkspaceRoot(cwd ?? this.workspaceRoot),
