  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Resolve the complete policy for one capability call. An approved explicit mode outranks the session\'s last `sandbox/mode` event, which outranks the deployment default — but only a session cwd names a writable workspace. Without one (an agentless call, or a session that never chose a directory) a `workspace-write` grant would fall back to the configured root, a directory nobody selected: the packaged desktop Host runs with its cwd at the user\'s home, so that grant would be home-wide. It therefore degrades to `read-only`, which admits no writable root, and the fallback root survives only as the reported boundary.\n\n`danger-full-access` is deliberately left alone. It grants no root-scoped write at all — the sandbox simply does not fence — so there is no fallback root to protect, and clamping it would silently reverse an explicit opt-out of the file sandbox without narrowing anything.',
