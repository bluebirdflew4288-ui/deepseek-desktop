...
describe('the provider hand-off', () => {
  ...
  it('degrades an agentless workspace-write default to read-only, still reporting the process.cwd() root', async () => {
    const { bash, calls } = await setup({ mode: 'workspace-write' })
    const result = await bash.run(bash.resolve({ command: 'true' }))
    // No session named a workspace, so the deployment default cannot lend its
    // write grant to the fallback root: the root is still reported, but nothing
