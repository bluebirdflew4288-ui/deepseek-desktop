...
function sandboxAgent(
  ...
  return {
    ...
    session: {
      id,
      // A tool call always runs for a session that chose its workspace; the
      // header carries that cwd, without which workspace-write degrades.
      header: { version: 0, id, createdAt: 0, cwd: '/sandbox-session-workspace' },
      events,
