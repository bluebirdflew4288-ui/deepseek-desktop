...
/** Stand-in for the shell's per-launch credential; only its opacity matters. */
const DESKTOP_TOKEN = 'per-launch-credential-0123456789'
...
async function mounted(config?: { trustedHosts?: string[]; apiToken?: string }): Promise<{
  routes: WebRoute[]
  upgrades: WebUpgradeRoute[]
...
describe('connection node half over a real HTTP server', () => {
   ...
   * browser would send it, and `authorization` carries a desktop credential.
   */
  ...
  it('refuses an unauthenticated loopback caller the trust fence alone would have served', async () => {
    // The finding this credential closes: a plain local client with a clean
    // Host header and no browser markers passed the fence and reached the
    // configuration plane. Configured, the credential refuses it — including on
    // the model catalog, which the fence deliberately leaves open to LAN.
    const { routes, dispose } = await mounted({ apiToken: DESKTOP_TOKEN })
    const { port, close } = await serve(routes)
    try {
      const host = `127.0.0.1:${String(port)}`
      for (const method of ['settings.describe', 'credentials.describe', 'agentPreset.read', 'llm.providers']) {
        expect([method, await call(port, method, host)]).toEqual([method, 401])
      }
    } finally {
      await close()
      await dispose()
  ...
  it('refuses a wrong credential and serves the right one', async () => {
    const { routes, dispose } = await mounted({ apiToken: DESKTOP_TOKEN })
    const { port, close } = await serve(routes)
    try {
      const host = `127.0.0.1:${String(port)}`
      expect(await call(port, 'settings.describe', host, `Bearer ${DESKTOP_TOKEN}x`)).toBe(401)
      // The right secret under the wrong scheme is still no credential.
      expect(await call(port, 'settings.describe', host, DESKTOP_TOKEN)).toBe(401)
      expect(await call(port, 'settings.describe', host, `Basic ${DESKTOP_TOKEN}`)).toBe(401)
      // 404 is the empty proxy's carrier answer: the credential was accepted.
      expect(await call(port, 'settings.describe', host, `Bearer ${DESKTOP_TOKEN}`)).toBe(404)
    } finally {
      await close()
      await dispose()
    }
  ...
  it('keeps the trust fence ahead of the credential, so a rebound authority stays 403', async () => {
    const { routes, dispose } = await mounted({ apiToken: DESKTOP_TOKEN })
    const { port, close } = await serve(routes)
    try {
      // A valid credential does not launder an authority the fence refuses:
      // the fence is evaluated first and answers 403, not 401.
      expect(await call(port, 'settings.describe', 'harness.example', `Bearer ${DESKTOP_TOKEN}`)).toBe(403)
    } finally {
      await close()
      await dispose()
    }
...
describe('the per-launch desktop credential on the WebSocket downlinks', () => {
  it('refuses an unauthenticated upgrade before protocol negotiation', async () => {
    const { upgrades, dispose } = await mounted({ apiToken: DESKTOP_TOKEN })
    const socket = new PassThrough()
    const chunks: Buffer[] = []
    socket.on('data', (chunk: Buffer) => { chunks.push(chunk) })
    const ended = once(socket, 'end')
