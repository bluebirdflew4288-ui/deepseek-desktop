...
import { hasApiToken } from './api-token.ts'
...
export interface ConnectionConfig {
  ...
  trustedHosts?: string[]
  /**
   * Per-launch bearer credential every `/api` request must carry. Leave it
   * unset for a standalone `dsh web`, whose browser client has no channel to
   * receive one. The desktop shell generates a credential per launch and
   * attaches it in its own request layer, so it never reaches renderer
   * JavaScript, disk, or a URL.
   */
  apiToken?: string
...
export const Config: z<ConnectionConfig> = z.object({
  ...
  // No default and no modifier: an absent credential leaves the standalone
  // `dsh web` surface exactly as upstream ships it.
  apiToken: z.string(),
 ...
 * configuration plane stays loopback-same-origin. A deployment that configures
 * `apiToken` additionally requires that per-launch credential on every
 * request, which is the authentication this fence disclaims.
 ...
 * pins them to loopback. A configured `apiToken` is required on top of both
 * fences, for the HTTP prefix and its WebSocket upgrades alike.
...
export function apply(ctx: Context, config?: ConnectionConfig): void {
  ...
  // Empty means unconfigured: the standalone surface keeps its upstream shape.
  ...
  const route: WebRoute = {
    ...
    handler: async (req, res) => {
      ...
      if (apiToken !== '' && !hasApiToken(req.headers, apiToken)) {
        res.writeHead(401)
