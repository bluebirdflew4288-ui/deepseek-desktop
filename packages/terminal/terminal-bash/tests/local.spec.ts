...
function stubAgent(ctx: Context, rawId: string, cwd?: string): Agent {
...
async function harness(
  ...
  // A real session carries its chosen workspace: without a cwd a
  // workspace-write policy degrades to read-only.
  const agent = stubAgent(ctx, `agent-${mode}`, root)
