...
async function boot(mode: SandboxMode): Promise<void> {
  ...
  // A tool call stamps the calling session's resolved policy on every write.
  // The agentless fallback is read-only now — no chosen workspace means no
  // write grant — so the suite supplies the policy it means to exercise.
...
describe('read-only', () => {
  ...
  it('denies edit of an existing file (the content is unchanged)', async () => {
    ...
    await expect(fs.editText(await target(path), { oldString: 'original', newString: 'changed', replaceAll: false }, undefined, undefined, policy))
...
describe('workspace-write containment', () => {
  ...
  it('a write under the workspace lands', async () => {
    ...
    const outcome = await fs.writeText(await target(path), 'inside', undefined, undefined, policy)
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    ...
    await fs.writeText(await target(path), 'temp', undefined, undefined, policy)
  ...
  it('an edit outside the workspace is denied; the original is untouched', async () => {
    ...
    await writeFile(path, 'original')
    await expect(fs.editText(await target(path), { oldString: 'original', newString: 'x', replaceAll: false }, undefined, undefined, policy))
      .rejects.toMatchObject({ code: 'FS_SANDBOX_DENIED' })
    expect(await readFile(path, 'utf8')).toBe('original')
  })
  ...
  it('an edit inside the workspace lands', async () => {
    ...
    await writeFile(path, 'original')
    const outcome = await fs.editText(await target(path), { oldString: 'original', newString: 'changed', replaceAll: false }, undefined, undefined, policy)
  ...
  it('mutates the freshly checked identity, not a stale outside targetKey (TOCTOU direction)', async () => {
    ...
    await fs.writeText(staleTarget, 'inside', undefined, undefined, policy)
  ...
  it('the workspace root itself passes the fence (path equal to a writable root), failing only on file type', async () => {
    ...
    await expect(fs.writeText(await target(workspace), 'x', undefined, undefined, policy)).rejects.toMatchObject({ code: 'FS_NOT_REGULAR_FILE' })
  })
})
...
describe('workspace-write with the filesystem root as the workspace (a root ending in the path separator)', () => {
  it('grants writes anywhere on that volume', async () => {
    ...
    try {
      ...
      await rootFs.writeText(await rootFs.resolve(path), 'anywhere', undefined, undefined, { mode: 'workspace-write', workspaceRoot: parse(base).root })
...
describe('danger-full-access', () => {
  ...
  it('writes anywhere, unfenced', async () => {
    ...
    await fs.writeText(await target(path), 'free', undefined, undefined, policy)
...
describe('FsError identity', () => {
  it('the denial is a structured FsError distinct from a host permission error', async () => {
    await boot('read-only')
    const error = await fs.writeText(await target(join(workspace, 'x.txt')), 'x', undefined, undefined, policy).catch((e: unknown) => e)
